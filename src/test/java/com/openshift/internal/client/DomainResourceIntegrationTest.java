...
public class DomainResourceIntegrationTest {
	...
	private static final String QUICKSTART_SPRING_EAP6_GITURL = "git://github.com/openshift/spring-eap6-quickstart.git"; 
	...
	@Test
	public void shouldCreateNonScalableApplicationWithInitialGitUrl() throws Exception {
		// pre-conditions
		...
		String applicationName =
				ApplicationTestUtils.createRandomApplicationName();
		IStandaloneCartridge jbosseap = LatestVersionOf.jbossEap().get(user);
		IApplication application =
				domain.createApplication(applicationName, jbosseap, QUICKSTART_SPRING_EAP6_GITURL);
		...
		assertThat(new ApplicationAssert(application))
				...
				.hasUUID()
				.hasCreationTime()
				.hasCartridge(jbosseap)
				.hasValidApplicationUrl()
				.hasValidGitUrl()
				.hasInitialGitUrl(QUICKSTART_SPRING_EAP6_GITURL)
				.hasEmbeddableCartridges()
				.hasAlias();
	}
	...
	public void shouldCreateNonScalableApplicationWithSmallGearAndGitUrl() throws Exception {
		...
		IStandaloneCartridge jbosseap = LatestVersionOf.jbossEap().get(user);
		IApplication application = domain.createApplication(
				applicationName, jbosseap, ApplicationScale.NO_SCALE, GearProfile.SMALL, QUICKSTART_SPRING_EAP6_GITURL);
